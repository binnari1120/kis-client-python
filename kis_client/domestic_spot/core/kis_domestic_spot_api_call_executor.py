...
from kis_client.domestic_spot.utilities.kis_domestic_spot_authentication_provider import \
    KoreaInvestmentSecuritiesDomesticSpotAuthenticationProvider
...
class KoreaInvestmentSecuritiesDomesticSpotApiCallExecutor:
    ...
    @kis_spot_single_process_rate_limited
    @kis_spot_inter_process_rate_limited
    async def execute_rest_api_call_async(self,
    ...
    #                                          endpoint: str,
    #                                          parameters: dict):
    #     request_url = f"{REST_API_HOST if not credentials.private_rest_proxy_host else credentials.private_rest_proxy_host}{endpoint}"
    #     query_string = self.__build_query_string(parameters=parameters)
    #     parameters[
    #         "signature"] = KoreaInvestmentSecuritiesDomesticSpotAuthenticationProvider.create_authentication_signature(
    #         query_string=query_string,
    #         private_key=credentials.private_key)
    #     # request_url += f"?{query_string}"
    #
    #     self._headers["X-MBX-APIKEY"] = credentials.public_key
    #
    #     async with aiohttp.ClientSession() as session:
    #         try:
